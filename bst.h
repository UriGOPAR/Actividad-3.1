...
public:
	...
	void remove(T);
	void removeAll();
	void givelevel(Node<T>*, int, std::stringstream &)const;
...
std::string BST<T>::byLevel() const {
	...
	if (!empty()) {
		//TO DO
		for (int n=0; n<=root->depth();n++){
			givelevel(root, n, aux);
		}
	}
	aux << "]";
	return aux.str();
...
template<class T>
void BST<T>::givelevel(Node<T> *root, int level,std::stringstream &aux)const{
	if (level==0){
		aux<<root->value<<" ";
	}
		...
		givelevel(root->left, level -1,aux);
		givelevel(root->right, level -1,aux);
	}
}
